...
# Model with vanilla VAE
class SensitivityModelVanillaVAE(pl.LightningModule):
    """
    Class implementing multi-task generative model for drug sensitivity prediction, with 
    standard, vanilla VAE as generative module.
    """
    ...
    def forward(self, drug_input, cell_line_input):
        ...
        input_rec, guiding_rec, z_sample, normal_rv, z_means, prior = vae_out
    ...
    def loss_function(self, batch_data, vae_out, aen_out, sensitivity_pred):
        # Unpack batch data
        batch_vae_input_X, batch_vae_guiding_X, batch_vae_guiding_classes, batch_aen_input_X, batch_targets, _, _ = batch_data   # SUSPECTLIBLE TO CHANGES
        ...
        # Compute VAE loss
        vae_loss = self.drug_model.loss_function(batch_vae_input_X, input_rec, batch_vae_guiding_X, guiding_rec, z_sample, normal_rv, prior)
    ...
    def training_step(self, train_batch, batch_idx):
        if self.vae_dataloader is not None:
            if self.total_step % self.vae_training_step_rate == 0:
                self.train_vae()
        # Unpack batch data
        # batch_vae_input_X, batch_vae_guiding_X, batch_vae_guiding_classes, batch_aen_input_X, batch_targets = train_batch
        ...
        # Unpack VAE loss
        neg_input_rec_likelihood, neg_guiding_rec_likelihood, entropy, prior_likelihood = vae_losses
    

        # Gather losses
        vae_loss = self.drug_model.input_rec_likelihood_weight * neg_input_rec_likelihood + self.drug_model.guiding_rec_likelihood_weight * neg_guiding_rec_likelihood - self.drug_model.entropy_weight * entropy - self.drug_model.prior_likelihood_weight * prior_likelihood
        # Overal loss
        ...
        self.log("prior_likelihood", prior_likelihood, on_step=False, on_epoch=True, prog_bar=True)
...
#         sensitivity_pred_corr = pearsonr(batch_targets.detach().cpu().numpy().reshape(-1),
#                                         sensitivity_pred.detach().cpu().numpy().reshape(-1))[0]
#         torch.corrcoef(torch.stack((a, b)))[0, 1]
        #print(batch_targets.shape, sensitivity_pred.shape)
